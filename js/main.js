...
function formatDate(dateString, format) {
    ...
    return format === 'day' ? days[d.getDay()] : monthNames[d.getMonth()];
}
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                searchLocation = normalizeCityName(data.city || 'Alex');
                displayWeather();
            } catch {
                searchLocation = 'Alex';
                displayWeather();
...
async function getWeather(searchLocation) {
    try {
        const response = await fetch(`${baseURL}?key=${APIKey}&q=${searchLocation}&days=3`);
...
async function displayWeather() {
    const weatherList = await getWeather(searchLocation);
    if (!weatherList) return;

    const { current: currentObj, location: locationObj, forecast: { forecastday: forecastArr } } = weatherList;

    const weatherContainer = `
        <!-- Today -->
        <div class="col-lg-4">
            <div class="forecast today rounded-3">
                <div id="today" class="forecast-header px-3 d-flex justify-content-between">
                    <div class="day fw-semibold">${formatDate(locationObj.localtime, 'day')}</div>
                    <div class="date text-light">${new Date(locationObj.localtime).getDate()} ${formatDate(locationObj.localtime, 'month')}</div>
                </div>
                <div id="current" class="forecast-content px-3">
                    ...
                    <div class="mt-4 d-flex justify-content-between">
                        <span class="fs-5"><img class="me-2" src="./imgs/icon-compass.png"" alt="">${currentObj.wind_degree}%</span>
                        <span class="fs-5"><img class="me-2" src="./imgs/icon-umberella.png" alt="">${currentObj.wind_kph}km/h</span>
                        <span class="fs-5"><img class="me-2" src="./imgs/icon-wind.png" alt="">${currentObj.wind_dir}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="tomorrow" class="col-lg-4 mt-3 mt-lg-0">
            <div class="text-center h-100 rounded-2 forecast tomorrow">
                <div class="forecast-header fw-semibold px-3">
                    <div class="day">${formatDate(forecastArr[1].date, 'day')}</div>
                </div>
        ...
        <div id="afterTomorrow" class="col-lg-4 mt-3 mt-lg-0">
            <div class="text-center h-100 rounded-2 forecast">
                <div class="forecast-header fw-semibold px-3">
                    <div class="day">${formatDate(forecastArr[2].date, 'day')}</div>
                </div>
                <div class="forecast-content px-3">
                    <div class="forecast-icon">
                        <i class="fa-solid fa-sun weather-icon"></i>
...
navLinks.forEach(link => {
    link.addEventListener('click', e => handleActiveNavLink(navLinks, e.target.parentNode));
});
